...
function adjacentSlots(slot) {
  const col = slot % 3, row = Math.floor(slot / 3), adj = [];
...
function simulate(pb, eb) {
  ...
  const ps = pb
    .map((u, i) => u ? { ...u, slot: i, side: 'p', alive: true, enraged: false } : null)
    .filter(Boolean);
  const es = eb
    .map((u, i) => u ? { ...u, slot: i, side: 'e', alive: true, enraged: false } : null)
    .filter(Boolean);
  ...
  function dealDmg(atk, tgt, d, isCrit = false) {
    ...
    if (tgt.hp <= 0) {
      tgt.alive = false;
      evs.push({ type: 'kill', uid: atk.id, tid: tgt.id, amt: dmg, isCrit, tick });
    } else {
      evs.push({ type: 'atk',  uid: atk.id, tid: tgt.id, amt: dmg, isCrit, tick });
    }
  }
  ...
  function applyBattleStart(units, foeSide) {
    ...
    units.filter(u => u.cid === 'paladin').forEach(u => {
      ...
      evs.push({ type: 'ability', uid: u.id, abilName: 'Sacred Aura', tick: -1 });
    });

    // Sneak Strike (Assassin) — instant hit on lowest-HP enemy at battle start
    units.filter(u => u.cid === 'assassin').forEach(u => {
      const targets = foeSide.filter(f => f.alive);
      if (!targets.length) return;
      const t = [...targets].sort((a, b) => a.hp - b.hp)[0];
      dealDmg(u, t, Math.floor(u.atk * u.skillPower), false);
      evs.push({ type: 'ability', uid: u.id, abilName: 'Sneak Strike', tick: -1 });
    });
  }
  ...
  function pickTarget(unit) {
    ...
    if (unitRow === 0) return bestInRow(0) || bestInRow(1) || bestInRow(2) || null;
    if (unitRow === 2) return bestInRow(2) || bestInRow(1) || bestInRow(0) || null;
    // Row B: own row first; then A vs C is 50/50
    const t = bestInRow(1);
    if (t) return t;
    const tA = bestInRow(0), tC = bestInRow(2);
  ...
  function emitQueue() {
    evs.push({ type: 'queue', order: queue.filter(u => u.alive).map(u => u.id), tick });
  }

  // ── Main loop ────────────────────────────────────────────────────────────────
  let safety = 2000;
  while (safety-- > 0) {
    const pa = ps.filter(u => u.alive), ea = es.filter(u => u.alive);
    ...
    if (unit.cid === 'healer') {
      ...
      const al = alliesOf(unit.side).filter(a => a.id !== unit.id && a.alive && a.hp < a.maxHp);
      if (al.length) {
      ...
      if (t && t.alive) {
        let dmg = unit.atk, isCrit = false;

        // Precise Shot (Archer) — extra crit chance
        const effCC = unit.critChance + (unit.cid === 'archer' ? unit.skillPower : 0);
        ...
        if (unit.cid === 'mage') {
          ...
          const splash = foes(unit.side).filter(f => f.id !== t.id && f.alive && adj.includes(f.slot));
          if (splash.length) {
            splash.forEach(f => dealDmg(unit, f, Math.floor(unit.atk * unit.skillPower)));
        ...
        if (unit.cid === 'archmage') {
          ...
          const goDeeper   = unit.side === 'p'
            ? f => (f.slot % 3) > primaryCol
            : f => (f.slot % 3) < primaryCol;
          const chain = foes(unit.side)
            .filter(f => f.id !== t.id && f.alive && Math.floor(f.slot / 3) === targetRow && goDeeper(f))
            .sort((a, b) => unit.side === 'p'
              ? (a.slot % 3) - (b.slot % 3)
              : (b.slot % 3) - (a.slot % 3));
          if (chain.length > 0) dealDmg(unit, chain[0], Math.floor(unit.atk * unit.skillPower));
          if (chain.length > 1) dealDmg(unit, chain[1], Math.floor((unit.atk * unit.skillPower) / 2));
        }
      }
    }
  ...
  let dmgP = 0, dmgE = 0, killsP = 0, killsE = 0;
  evs
    .filter(e => e.tick >= 0 && (e.type === 'atk' || e.type === 'kill'))
    .forEach(e => {
      const u = [...ps, ...es].find(x => x.id === e.uid);
      if (!u) return;
      if (u.side === 'p') { dmgP += e.amt; if (e.type === 'kill') killsP++; }
      else                { dmgE += e.amt; if (e.type === 'kill') killsE++; }
    });
